...
- **РАБОТА С ЭЛЕМЕНТАМИ |HIDDEN**: Элементы с индикатором `|HIDDEN` (например, `[888]|HIDDEN`) кликабельны, но не видны на экране.
  ...
  * Если элемент имеет атрибуты интерактивности (`role=button`, `type=submit` и т.д.), он может быть кликабельным даже если помечен `|HIDDEN`
...
- **РАБОТА С ПОЧТОВЫМИ ИНТЕРФЕЙСАМИ**: Если вы работаете с почтовым клиентом, помните общие принципы:
  ...
  - Эти знания - только общая информация о структуре почтовых интерфейсов. Вы сами решаете, какие действия предпринимать для выполнения задачи. Анализируйте DOM и принимайте решения самостоятельно на основе текущего состояния страницы.
...
<memory_examples>
"memory": "Посетил 2 из 5 целевых веб-сайтов. Собрал данные о ценах с первого сайта ($39.99) и второго ($42.00). Все еще нужно проверить оставшиеся 3 сайта для сравнения товаров."
