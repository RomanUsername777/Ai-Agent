...
try:
	from agent.browser.cloud.cloud import CloudBrowserAuthError, CloudBrowserClient, CloudBrowserError
except ImportError:
...
try:
	from agent.browser.cloud.views import CloudBrowserParams, CreateBrowserRequest, ProxyCountryCode
except ImportError:
	# Create dummy classes if cloud not available
...
from agent.browser.profile import BrowserProfile, ProxySettings
from agent.browser.views import BrowserStateSummary, TabInfo
from agent.dom.views import DOMRect, EnhancedDOMTreeNode, TargetInfo
from agent.observability import observe_debug
from agent.utils import _log_pretty_url, create_task_with_error_handling, is_new_tab_page

if TYPE_CHECKING:
	from agent.actor.page import Page
	from agent.browser.demo_mode import DemoMode

DEFAULT_BROWSER_PROFILE = BrowserProfile()

_LOGGED_UNIQUE_SESSION_IDS = set()  # Отслеживаем уникальные ID сессий, которые были залогированы, чтобы всегда присваивать достаточно уникальный ID новым сессиям и избегать неоднозначности в логах
red = '\033[91m'
reset = '\033[0m'
...
class BrowserSession(BaseModel):
	...
	def logger(self) -> Any:
		...
		# 	self._logger = logging.getLogger(f'agent.{self}')
		return logging.getLogger(f'agent.{self}')
	...
	def model_post_init(self, __context) -> None:
		...
		from agent.browser.watchdog_base import BaseWatchdog
	...
	async def kill(self) -> None:
		...
		from agent.browser.events import SaveStorageStateEvent
		...
		try:
			...
			if not self.cdp_url:
				if self.browser_profile.use_cloud or self.browser_profile.cloud_browser_params is not None:
					...
					# Упрощённая версия: облачный браузер не используется
	...
	async def on_NavigateToUrlEvent(self, event: NavigateToUrlEvent) -> None:
		...
		if not self.agent_focus_target_id:
			self.logger.warning('Невозможно перейти - браузер не подключен')
			return
		...
		try:
			...
			self.logger.error(f'Навигация не удалась: {type(e).__name__}: {e}')
	...
	async def _navigate_and_wait(self, url: str, target_id: str, timeout: float | None = None) -> None:
		...
		# Timeout - continue anyway with diagnostics (но без жёстких WARNING в консоль)
		duration_ms = (asyncio.get_event_loop().time() - nav_start_time) * 1000
		if not seen_events:
			self.logger.error(
			...
			# В упрощённой версии не засоряем вывод WARNING-сообщениями, оставляем только debug
			self.logger.debug(f'⚠️ Page readiness timeout ({timeout}s, {duration_ms:.0f}ms) for {url}')
	...
	async def new_page(self, url: str | None = None) -> 'Page':
		...
		from agent.actor.page import Page as Target
	...
	async def get_pages(self) -> list['Page']:
		...
		# Import here to avoid circular import
		from agent.actor.page import Page as PageActor
	...
	async def attach_all_watchdogs(self) -> None:
		...
		# from agent.browser.crash_watchdog import CrashWatchdog
		from agent.browser.watchdogs.default_action_watchdog import DefaultActionWatchdog
		from agent.browser.watchdogs.dom_watchdog import DOMWatchdog
		from agent.browser.watchdogs.downloads_watchdog import DownloadsWatchdog
		from agent.browser.watchdogs.local_browser_watchdog import LocalBrowserWatchdog
		from agent.browser.watchdogs.popups_watchdog import PopupsWatchdog
		from agent.browser.watchdogs.recording_watchdog import RecordingWatchdog
		from agent.browser.watchdogs.security_watchdog import SecurityWatchdog
		from agent.browser.watchdogs.storage_state_watchdog import StorageStateWatchdog
		...
		# Initialize DOMWatchdog (handles building the DOM tree and detecting interactive elements)
		...
		# Register ScreenshotEvent handler
		self.event_bus.on(ScreenshotEvent, self._on_ScreenshotEvent)
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		try:
			...
			from agent.browser.session_manager import SessionManager
			...
			from agent.utils import is_new_tab_page
	...
	async def navigate_to(self, url: str, new_tab: bool = False) -> None:
		...
		"""
		from agent.browser.events import NavigateToUrlEvent
	...
	async def get_dom_element_at_coordinates(self, x: int, y: int) -> EnhancedDOMTreeNode | None:
		...
		"""
		from agent.dom.views import NodeType
	...
	async def remove_highlights(self) -> None:
		...
		try:
			...
			(function() {
				...
				console.log('Removing', highlights.length, 'agent highlight elements');
	...
	async def add_highlights(self, selector_map: dict[int, 'EnhancedDOMTreeNode']) -> None:
		...
		try:
			...
			(function() {{
				...
				const existingContainer = document.getElementById('agent-debug-highlights');
				...
				const strayHighlights = document.querySelectorAll('[data-agent-highlight]');
				...
				interactiveElements.forEach((element, index) => {{
					...
					highlight.setAttribute('data-agent-highlight', 'element');
