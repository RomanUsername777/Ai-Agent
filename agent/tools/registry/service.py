...
from agent.tools.registry.views import (
	ActionModel,
	ActionRegistry,
	RegisteredAction,
	SpecialActionParameters,
)
from agent.utils import is_new_tab_page, match_url_with_domain_pattern, time_execution_async
		...
		# Meaning the length is more than 1
			...
			class ActionModelUnion(RootModel[union_type]):  # type: ignore
				...
				def model_validate(cls, obj, *, strict=None, from_attributes=None, context=None):
					"""Кастомная валидация: пробуем каждый тип в Union по очереди"""
					# Пробуем каждый тип в Union
					errors = []
					for model_type in individual_action_models:
						try:
							# Пытаемся распарсить как этот тип
							parsed = model_type.model_validate(obj, strict=strict, from_attributes=from_attributes, context=context)
							# Если успешно, создаем RootModel с этим значением
							return cls(root=parsed)
						except Exception as e:
							errors.append((model_type.__name__, str(e)))
							continue
					
					# Если ни один тип не подошел, пробрасываем первую ошибку
					if errors:
						first_error = errors[0]
						error_msg = f'Failed to parse as any action type. First error ({first_error[0]}): {first_error[1][:200]}'
